...
{   
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        $categories = [];
        $categoriesNames = ['Femme', 'Homme', 'Enfant', 'Equipement', 'Nutrition', 'Soldes'];
        ...
        for ($i = 0; $i < 6; $i++) {
            ...
            // set category display order value
        ...
        $subCategories = [];
        $subCategoriesNames = ['Vétements', 'Chaussures', 'Accessoires', 'Soldes'];

        $nutritionSubCat = [];
        $nutritionSubCatNames = ['Récupération', 'Effort', 'Sèche', 'Masse', 'Soldes'];
        
        $equipementSubCat = [];
        $equipementSubCatNames = ['Vélo', 'Course', 'Musculation', 'Natation', 'Camping'];
        ...
        foreach($categories as $cat) {
            ...
            if ($cat->getName() === 'Femme' || $cat->getName() === 'Homme' || $cat->getName() === 'Enfant') {
                ...
                for ($i = 0; $i < 4; $i++) {
                    ...
                    $subCat->setName($subCategoriesNames[$i]);

                    // set subcategory display order value
            ...
            if ($cat->getName() === 'Nutrition') {
                for ($j = 0; $j < 5; $j++) {
                    ...
                    $subCat = new SubCategory();
                    $subCat->setName($nutritionSubCatNames[$j]);
            ...
            if ($cat->getName() === 'Equipement') {
                for ($k = 0; $k < 5; $k++) {
                    ...
                    $subCat = new SubCategory();
                    $subCat->setName($equipementSubCatNames[$k]);
...
//* EQUIPEMENT *//
        ...
        for( $i = 0; $i < 10; $i++){
            ...
            $course->setCategory($categories[3]);
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            $recuperation->setCategory($categories[4]);
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            $musculation->setCategory($categories[3]);
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            $natation->setCategory($categories[3]);
            $natation->setProductType($natationTypes[rand(0, count($natationTypes) - 1)]);
        ...
        for ($i = 0; $i < 3; $i++) {
         ...
         for ($i = 0; $i < 10; $i++) {
            ...
            // categories ['Femme', 'Homme', 'Enfant', 'Equipement', 'Nutrition', 'Soldes'];
             $camping->setCategory($categories[3]);
...
//* VETEMENTS *//
        ...
        for ($i = 0; $i < 3; $i++) {
            ...
            $productType->setName($names[$i]);
            $productType->addSubCategory($subCategories[rand(0, count($subCategories) - 1)]);
        ...
        $vetement->setCategory($categories[rand(0, 2)]);
        $vetement->setProductType($vetementTypes[rand(0, count($vetementTypes) - 1)]);
        $vetement->setSubCategory($subCategories[rand(0, count($subCategories) - 1)]);
...
//TODO  debuger ça ! tout se met dans la categorie Femme et dans la sous categorie vetements ?
        ...
        //INFO : $subCategories = ['Vétements', 'Chaussures', 'Accessoires', 'Soldes'];
        // if productType is 'baskets' then add to subCat 'chaussures' if productType is tee-shirt or jean then add to subCat 'vetements'
        // $vetement->getProductType()->getName() === 'baskets' ? $vetement->setSubCategory($subCategories[2]) : '';
        // $vetement->getProductType()->getName() === 'tea-shirt' ? $vetement->setCategory($categories[1]) : '';
        // $vetement->getProductType()->getName() === 'jean' ? $vetement->setCategory($categories[1]) : '';
